...
socket.on("message", function(msg) { let chatBox = document.getElementById("chat-box"); chatBox.innerHTML += <p>${msg}</p>; chatBox.scrollTop = chatBox.scrollHeight; });

function logout() { fetch("/logout").then(() => { document.getElementById("chat-container").style.display = "none"; document.getElementById("login-container").style.display = "block"; }); }
